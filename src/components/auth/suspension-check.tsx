...
console.log('🔥 SuspensionCheck module loaded!');
...
export default function SuspensionCheck({ children, mode = 'modal' }: SuspensionCheckProps) {
  console.log('SuspensionCheck: Component mounted with mode:', mode);
  ...
  console.log('SuspensionCheck: session status:', status, 'user:', session?.user?.email);

  useEffect(() => {
    const checkSuspensionStatus = async () => {
      if (status === 'loading' || !session?.user?.id) {
        console.log('SuspensionCheck: Skipping check - status:', status, 'user:', session?.user?.id);
        return;
      }

      console.log('SuspensionCheck: Starting suspension check for user:', session.user.email);

      try {
        // Check user suspension/ban status from the API
        const response = await fetch('/api/auth/suspension-status');
        console.log('SuspensionCheck: API response status:', response.status);
        
        if (response.ok) {
          const data = await response.json();
          console.log('SuspensionCheck: API response data:', data);
          setSuspensionData(data);
          
          // Banned users should have been blocked at login, but double-check
          if (data.isBanned) {
            console.log('SuspensionCheck: User is banned, redirecting to login');
            window.location.href = '/login?error=banned';
            return;
          }
          
          if (data.isSuspended) {
            console.log('SuspensionCheck: User is suspended, showing modal');
          } else {
            console.log('SuspensionCheck: User is not suspended');
          }
        } else {
          const errorText = await response.text();
          console.error('SuspensionCheck: API error:', response.status, errorText);
        }
      } catch (error) {
        console.error('Error checking suspension status:', error);
      } finally {
        setSuspensionChecked(true);
        console.log('SuspensionCheck: Check completed');
      }
    };
  ...
  if (!suspensionChecked && status !== 'unauthenticated') {
    console.log('SuspensionCheck: Showing loading spinner');
    return (
  ...
  console.log('SuspensionCheck: Render check - isSuspended:', suspensionData.isSuspended, 'modalDismissed:', modalDismissed, 'mode:', mode);

  // If user is suspended, show appropriate UI based on mode
  if (suspensionData.isSuspended && !modalDismissed) {
    console.log('SuspensionCheck: Should show suspension UI');
    if (mode === 'modal') {
      console.log('SuspensionCheck: Rendering suspension modal');
      return (
        <>
